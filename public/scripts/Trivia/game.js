...
async function get_question () {
    ...
    try {
        const data = await $.ajax({ url: `https://opentdb.com/api.php`, data: { amount: 1, category, difficulty, type, token } });
        const time = new Date().getSeconds()
        const question = data.results[0]
        $('.GameName').html(question.question)
        const options = [question.correct_answer, ...question.incorrect_answers].sort(() => Math.random() - 0.5).sort(() => Math.random() - 0.5)
