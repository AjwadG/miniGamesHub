...
function check() {
    ...
    if (col == lenght + 1 && row <= lenght) {
        $.ajax({ url, type: 'POST', data: JSON.stringify({ lenght, number, guess: word.join('') , row}), contentType: 'application/json; charset=utf-8' }).done((data) => {
            ...
            if (data.won)
                ...
                new Audio(`sounds/Wordle/${Colors[row % 3]}.mp3`).play();
        })
    }
