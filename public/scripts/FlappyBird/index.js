...
function collision (player, objects) { 
    for (let i = 0; i < objects.length; i++) {
        ...
        if (player.x + player.width >= object.x && player.x <= object.x + object.width) {
            if ( player.y <= object.y + object.height && player.y + player.height >= object.y ) {
            return true;
            } else {
                point.play()
            }
...
function align () {
    const height = $('.pipe_top').height();
    const res = Array.from({length: 3}, _ => Math.floor(Math.random()  * (height - 450 + 1)) + 300);
